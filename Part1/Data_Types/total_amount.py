...
def main():
    ...
    try:
        ...
        mapped_y_data = y_data.map(lambda x: check_total_amount(x[18]))
        print("SAMPLE YELLOW CAB DATA OUTPUT: \n")
        print(mapped_y_data.take(20))
        
        g_data = gd.gd_processing(sc, green_data_path)
        mapped_g_data = g_data.map(lambda x: check_total_amount(x[18]))
        print("SAMPLE GREEN CAB DATA OUTPUT: \n")
        print(mapped_g_data.take(20))
        
        print("saving all outputs to files")
        mapped_y_data.saveAsTextFile("total_amount_y.out")
        mapped_g_data.saveAsTextFile("total_amount_g.out")
        sc.stop()
